...
- `clients/p2p-gui/`：Rust `egui` 桌面客户端（聊天 + 可续传文件传输 + Windows 远程桌面）。
...
- 画面由 DXGI Desktop Duplication 捕获，缩放到不超过 1280×720，以 128×128 RGBA 块比较变化并使用快速 PNG 编码。每帧使用独立 QUIC 单向流，最多保留一个待发送帧；控制流、画面流和文件流按 `100 / 10 / -10` 设置优先级。
- 接收端只保留最新压缩帧和最新解码帧，关键帧用于首次显示、尺寸变化和超时恢复；静止画面不会持续发送像素数据。
- 鼠标、滚轮、扫描码和 Unicode 文本通过现有加密控制流传输。观看方点击画面后接管键盘，按 `Esc` 释放本地控制焦点。
- Windows 实现只直接依赖微软 `windows` API 投影；PNG 复用 `eframe` 已有的 `image/png` 依赖，不引入第三方采集、输入、WebRTC 或视频编解码库。
...
- 远程桌面首版仅支持 Windows、单显示器和最高 1280×720 / 15 FPS；暂不包含音频、剪贴板、多显示器拼接、无人值守或硬件视频编码。
