...
- `clients/p2p-core/`：Rust 核心库——信令协议、会话编排、文件传输与远程桌面协议。
- `clients/p2p-gui/`：Rust `egui` 桌面客户端（聊天 + 可续传文件传输 + Windows/macOS 远程桌面）。
...
| **直连 QUIC**（客户端 ↔ 客户端） | 聊天消息、文件控制/分块、远程桌面画面与输入 | 打洞成功后建立；断开即会话结束（见「失败策略」） |
...
enum P2pMessage {
    ...
    RemoteDesktop { message: RemoteDesktopControl },
}
```
...
| **Phase 6** | **Windows 远程桌面**：能力协商、DXGI Desktop Duplication、PNG 差分块、QUIC 单向帧流、逐次授权与 `SendInput` 临时控制 | 两台 Windows 客户端可在直连后共享/观看单个显示器；权限可随时撤销；远程桌面停止不影响聊天和文件 | ✅ 代码与自动化测试完成；双机体验待手动验收 |
| **Phase 7** | **macOS 远程桌面**：ScreenCaptureKit、CoreGraphics 键鼠注入、TCC 权限预检与轻量 `.app` 包装 | Mac 可与 Windows/Mac 互相共享和观看；仅开启控制时请求辅助功能权限；旧 Windows 邀请保持兼容 | ✅ 代码与自动化测试完成；跨设备体验待手动验收 |
...
构建产物为 `clients/target/<mode>/P2P Signaling.app`，最低支持 macOS 12.3；
`scripts/start-client-macos.command` 支持从 Finder 双击启动。默认使用临时签名；如需让重建后的
屏幕录制授权稳定关联到开发者身份，可设置 `P2P_MACOS_CODESIGN_IDENTITY` 后再构建。
...
- 双方在 `ConnectInfo.capabilities` 中协商 `remote-desktop-v1`；旧客户端仍可继续聊天和传文件，但不会显示共享入口。
- 共享方选择一个显示器并发起邀请，观看方必须明确接受。控制权限默认关闭，只在当前会话中由共享方显式开启；停止共享、撤销权限或直连断开时会释放所有已按下的键鼠状态。
- Windows 使用 DXGI Desktop Duplication；macOS 12.3+ 使用 ScreenCaptureKit。两端都输出不超过 1280×720，以 128×128 RGBA 块比较变化并使用快速 PNG 编码。每帧使用独立 QUIC 单向流，最多保留一个待发送帧；控制流、画面流和文件流按 `100 / 10 / -10` 设置优先级。
- 接收端只保留最新压缩帧和最新解码帧，关键帧用于首次显示、尺寸变化和超时恢复；静止画面不会持续发送像素数据。
- 鼠标、滚轮、扫描码和 Unicode 文本通过现有加密控制流传输。观看方点击画面后接管键盘，按 `Esc` 释放本地控制焦点。
- macOS 第一次共享前会请求“屏幕录制”权限，授权后需重启应用；只有勾选“允许对方临时控制”时才请求“辅助功能”权限。运行中撤销控制权限只停止输入注入，不停止画面、聊天或文件传输。
- Windows 只直接依赖微软 `windows` API 投影；macOS 复用 `eframe` 已有的 `objc2` 依赖族并仅启用 ScreenCaptureKit/CoreGraphics 所需绑定。PNG 继续复用现有 `image/png`，不引入第三方采集、输入、WebRTC 或视频编解码库。
...
- **远程控制授权**：画面邀请需要观看方接受，控制还需要共享方单独授权；授权不持久化，并在停止或断线时撤销。
...
- 远程桌面支持 Windows 与 macOS 12.3+，仍只共享单个显示器且最高 1280×720 / 15 FPS；暂不包含音频、剪贴板、多显示器拼接、无人值守或硬件视频编码。
- Windows UIPI 会阻止普通权限客户端向更高完整性级别窗口注入输入；UAC 安全桌面和 `Ctrl+Alt+Del` 不在支持范围内。
- macOS 的安全输入、登录窗口、受保护内容以及未获 TCC 授权的应用不在远程控制范围内；临时签名在二进制重建后可能需要重新授权。
- 观看端按住 AltGr（Ctrl+Alt）输入的国际布局特殊字符不会转发；标点组合键按美式物理键位映射，非美式布局下可能偏移。
